...
links = []
with open("phone_links1.txt", "r") as f:
    for line in f:
        if re.search('oppo', line[:-1]):
            links.append(line[:-1])
print(len(links))

filepath = 'specs_oppo.csv'
...
cam1video= [];  cam2modules= [];  cam2features= []; cam2video= []; jack= [];  wlan= [];  bluetooth= [];
...
specs= {'Name':name, '2G':band_2G, '3G':band_3G, '4G':band_4G, '5G':band_5G, 'Announced':announced, 'Released':released,
        ...
        'Front Camera(Video)':cam2video, 'Headphone Jack':jack, 'WiFi':wlan, 'Bluetooth':bluetooth,
        ...
        'Battery Test':battery_test, 'Rating':review}
